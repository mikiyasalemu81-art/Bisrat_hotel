/**
 * Bisrat Hotel — Full Digitized Menu Catalog (102 Items across 13 Categories)
 * Contains exact prices in ETB, trilingual names, descriptions, 
 * and authentic culinary photography. Handcrafted by manual coding.
 */

export const CATEGORY_CONFIG = [
  {
    id: "wine",
    nameEn: "Wine",
    nameAm: "የወይን ጠጅ",
    nameOr: "Wayinii",
    icon: "Wine",
    description: "Curated collection of premier Ethiopian and regional wines.",
  },
  {
    id: "cold_beverages",
    nameEn: "Cold Beverages",
    nameAm: "ቀዝቃዛ መጠጦች",
    nameOr: "Dhugaatii Qabbanaawaa",
    icon: "Beer",
    description: "Chilled bottled waters, sparkling minerals, soft drinks & cold beers.",
  },
  {
    id: "pizza",
    nameEn: "Pizza Corner",
    nameAm: "የፒዛ ዝርዝር",
    nameOr: "Pitsaa Addaa",
    icon: "Pizza",
    description: "Stone-baked artisanal pizzas with rich cheeses and fresh savory toppings.",
  },
  {
    id: "burger",
    nameEn: "Burger",
    nameAm: "በርገር",
    nameOr: "Bargerii",
    icon: "Sandwich",
    description: "Juicy beef and crispy chicken burgers served with gourmet toppings.",
  },
  {
    id: "chicken",
    nameEn: "Chicken Corner",
    nameAm: "የዶሮ ምግቦች",
    nameOr: "Nyaata Lukkuu",
    icon: "Drumstick",
    description: "Crispy nuggets, glazed wings, grilled and seasoned whole roasts.",
  },
  {
    id: "pasta",
    nameEn: "Pasta",
    nameAm: "ፓስታ እና ማካሮኒ",
    nameOr: "Paastaa fi Maakaroonii",
    icon: "UtensilsCrossed",
    description: "Authentic Italian style pastas tossed in rich sauces and fresh herbs.",
  },
  {
    id: "traditional",
    nameEn: "Traditional Corner",
    nameAm: "የሀበሻ ባህላዊ ምግቦች",
    nameOr: "Nyaata Aadaa",
    icon: "Flame",
    description: "Heritage Ethiopian stews, sizzling tibs, and grand combo platters on injera.",
  },
  {
    id: "soup",
    nameEn: "Soup Corner",
    nameAm: "ሾርባዎች",
    nameOr: "Shorbaa",
    icon: "Soup",
    description: "Warm, nourishing chef-crafted broths and creamy vegetable purees.",
  },
  {
    id: "salad",
    nameEn: "Salad Corner",
    nameAm: "ሰላጣዎች",
    nameOr: "Salaaxaa",
    icon: "Salad",
    description: "Crisp garden greens, avocado bowls, and fresh proteins with zesty dressings.",
  },
  {
    id: "fish",
    nameEn: "Fish Corner",
    nameAm: "የዓሣ ምግቦች",
    nameOr: "Nyaata Qurxummii",
    icon: "Fish",
    description: "Freshly prepared lake fish cutlets, golden chips, and seasoned grilled fish.",
  },
  {
    id: "breakfast",
    nameEn: "Breakfast",
    nameAm: "የቁርስ ምግቦች",
    nameOr: "Ciree",
    icon: "Egg",
    description: "Energizing morning staples: Chechebsa, omelets, firfir varieties & pancakes.",
  },
  {
    id: "juice",
    nameEn: "Juice Corner",
    nameAm: "ትኩስ ጁስ እና ሼክ",
    nameOr: "Juusii fi Sheekii",
    icon: "Citrus",
    description: "100% natural tropical fruit smoothies, spris layers, and healthy elixirs.",
  },
  {
    id: "hot_drink",
    nameEn: "Hot Drink",
    nameAm: "ትኩስ መጠጦች",
    nameOr: "Dhugaatii Ho'aa",
    icon: "Coffee",
    description: "Traditional Jebena brews, aromatic macchiatos, and spiced infused teas.",
  }
];

export const initialMenuItems = [
  // ==========================================
  // 1. WINE (4 items)
  // ==========================================
  {
    id: "wine-1",
    nameEn: "Awash wine",
    nameAm: "አዋሽ ወይን",
    nameOr: "Wayinii Awaash",
    category: "wine",
    price: 1000,
    isAvailable: true,
    placeholderSlot: "menu-awash-wine.jpg",
    imageUrl: "/images/01_Awash_White_Wine.jpg",
    description: "Classic Ethiopian red wine crafted from Awash Valley grapes, featuring deep berry aromas with a smooth finish.",
  },
  {
    id: "wine-2",
    nameEn: "Axumite wine",
    nameAm: "አክሱማይት ወይን",
    nameOr: "Wayinii Aksumite",
    category: "wine",
    price: 1200,
    isAvailable: true,
    placeholderSlot: "menu-axumite-wine.jpg",
    imageUrl: "/images/09_Axumit_Red_Wine.png",
    description: "Renowned sweet Ethiopian dessert wine with gentle floral notes and a rich, velvety ruby hue.",
  },
  {
    id: "wine-3",
    nameEn: "Kamila wine",
    nameAm: "ካሚላ ወይን",
    nameOr: "Wayinii Kamiilaa",
    category: "wine",
    price: 1200,
    isAvailable: true,
    placeholderSlot: "menu-kamila-wine.jpg",
    imageUrl: "/images/04_Kenila_Rose_and_White_Wine.png",
    description: "Elegantly balanced dry red wine with hints of oak and subtle spice, poured into a crystal goblet.",
  },
  {
    id: "wine-4",
    nameEn: "Acacia wine",
    nameAm: "አካሲያ ወይን",
    nameOr: "Wayinii Akaasiyaa",
    category: "wine",
    price: 2800,
    isAvailable: true,
    placeholderSlot: "menu-acacia-wine.jpg",
    imageUrl: "/images/03_Acacia_Dry_Red_Wine.jpg",
    description: "Premium reserve Ethiopian wine from Castel Winery, displaying complex tannic structure and refined elegance.",
  },

  // ==========================================
  // 2. COLD BEVERAGES (9 items)
  // ==========================================
  {
    id: "cold-1",
    nameEn: "1L water",
    nameAm: "1 ሊትር የታሸገ ውሃ",
    nameOr: "Bishaan Saamunaa 1L",
    category: "cold_beverages",
    price: 75,
    isAvailable: true,
    placeholderSlot: "menu-1l-water.jpg",
    imageUrl: "/images/cold-water-1l.jpg",
    description: "Chilled 1-liter sealed pure natural mineral water bottle.",
  },
  {
    id: "cold-2",
    nameEn: "1/2L water",
    nameAm: "0.5 ሊትር የታሸገ ውሃ",
    nameOr: "Bishaan Saamunaa 0.5L",
    category: "cold_beverages",
    price: 60,
    isAvailable: true,
    placeholderSlot: "menu-half-l-water.jpg",
    imageUrl: "/images/07_TOP_Mineral_Water.png",
    description: "Compact 500ml chilled natural spring water bottle.",
  },
  {
    id: "cold-3",
    nameEn: "Big water",
    nameAm: "ትልቅ የታሸገ ውሃ (2L)",
    nameOr: "Bishaan Guddaa",
    category: "cold_beverages",
    price: 90,
    isAvailable: true,
    placeholderSlot: "menu-big-water.jpg",
    imageUrl: "/images/08_Gold_Water_Bottle.jpg",
    description: "Extra large bottle of crisp pure drinking water for tables and sharing.",
  },
  {
    id: "cold-4",
    nameEn: "Soft drink",
    nameAm: "ለስላሳ መጠጥ",
    nameOr: "Dhugaatii Lallaafaa",
    category: "cold_beverages",
    price: 90,
    isAvailable: true,
    placeholderSlot: "menu-soft-drink.jpg",
    imageUrl: "/images/11_Sprite_Fanta_Coca_Cola_Bottles.jpg",
    description: "Chilled refreshing soft drink served ice cold in a glass bottle with ice.",
  },
  {
    id: "cold-5",
    nameEn: "Ambo water",
    nameAm: "አምቦ ውሀ",
    nameOr: "Bishaan Amboo",
    category: "cold_beverages",
    price: 70,
    isAvailable: true,
    placeholderSlot: "menu-ambo-water.jpg",
    imageUrl: "/images/15_Ambo_Sparkling_Water.jpg",
    description: "Ethiopia's iconic Ambo sparkling mineral water, naturally carbonated and ice-cold.",
  },
  {
    id: "cold-6",
    nameEn: "Normal beer",
    nameAm: "መደበኛ ድራፍት ቢራ",
    nameOr: "Biiraa Idilee",
    category: "cold_beverages",
    price: 130,
    isAvailable: true,
    placeholderSlot: "menu-normal-beer.jpg",
    imageUrl: "/images/18_Beer_Mug.jpg",
    description: "Authentic chilled Ethiopian lager beer (St. George / Giorgis) poured with a thick frothy collar.",
  },
  {
    id: "cold-7",
    nameEn: "Bedele special beer",
    nameAm: "በደሌ ስፔሻል ቢራ",
    nameOr: "Biiraa Baddeellee Addaa",
    category: "cold_beverages",
    price: 150,
    isAvailable: true,
    placeholderSlot: "menu-bedele-beer.jpg",
    imageUrl: "/images/02_Bedele_Special_Beer.webp",
    description: "Famous Ethiopian Bedele Special premium export-quality lager brewed with 100% natural ingredients.",
  },
  {
    id: "cold-8",
    nameEn: "Arada beer",
    nameAm: "አራዳ ቢራ",
    nameOr: "Biiraa Araadaa",
    category: "cold_beverages",
    price: 130,
    isAvailable: true,
    placeholderSlot: "menu-arada-beer.jpg",
    imageUrl: "/images/17_Arada_Beer_Illustrated_Bottles.jpg",
    description: "Authentic Ethiopian Arada refreshing drink crafted with natural fruit essences and sparkling water.",
  },
  {
    id: "cold-9",
    nameEn: "Heineken beer",
    nameAm: "ሃይኒከን ቢራ",
    nameOr: "Biiraa Haayniken",
    category: "cold_beverages",
    price: 160,
    isAvailable: true,
    placeholderSlot: "menu-heineken-beer.jpg",
    imageUrl: "/images/12_Heineken_Beer_Bottles.jpg",
    description: "International premium pale lager served chilled in a cold glass with crisp foam.",
  },

  // ==========================================
  // 3. PIZZA CORNER (6 items)
  // ==========================================
  {
    id: "pizza-1",
    nameEn: "Chicken pizza",
    nameAm: "የዶሮ ፒዛ",
    nameOr: "Pitsaa Lukkuu",
    category: "pizza",
    price: 750,
    isAvailable: true,
    placeholderSlot: "menu-chicken-pizza.jpg",
    imageUrl: "https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?auto=format&fit=crop&w=1200&q=85",
    description: "Oven-baked crust topped with seasoned grilled chicken strips, stretchy mozzarella, and oregano.",
  },
  {
    id: "pizza-2",
    nameEn: "Margarita pizza",
    nameAm: "ማርጋሪታ ፒዛ",
    nameOr: "Pitsaa Maargariitaa",
    category: "pizza",
    price: 520,
    isAvailable: true,
    placeholderSlot: "menu-margarita-pizza.jpg",
    imageUrl: "https://images.unsplash.com/photo-1604382355076-af4b0eb60143?auto=format&fit=crop&w=1200&q=85",
    description: "Traditional Italian recipe with crushed ripe plum tomatoes, fresh mozzarella, and aromatic basil leaves.",
  },
  {
    id: "pizza-3",
    nameEn: "Special Bisrat pizza",
    nameAm: "ልዩ ብስራት ፒዛ",
    nameOr: "Pitsaa Addaa Bisrat",
    category: "pizza",
    price: 720,
    isAvailable: true,
    placeholderSlot: "menu-special-bisrat-pizza.jpg",
    imageUrl: "https://images.unsplash.com/photo-1534308983496-4fabb1a015ee?auto=format&fit=crop&w=1200&q=85",
    description: "House masterpiece loaded with seasoned meats, bell peppers, sautéed mushrooms, olives, and double cheese.",
  },
  {
    id: "pizza-4",
    nameEn: "Tuna pizza",
    nameAm: "የቱና ዓሣ ፒዛ",
    nameOr: "Pitsaa Qurxummii Tuunaa",
    category: "pizza",
    price: 670,
    isAvailable: true,
    placeholderSlot: "menu-tuna-pizza.jpg",
    imageUrl: "https://images.unsplash.com/photo-1574071318508-1cdbab80d002?auto=format&fit=crop&w=1200&q=85",
    description: "Flaked oceanic tuna with caramelized onions, tangy capers, and golden melted cheese.",
  },
  {
    id: "pizza-5",
    nameEn: "Vegetable pizza",
    nameAm: "የአታክልት ፒዛ",
    nameOr: "Pitsaa Kuduraa",
    category: "pizza",
    price: 490,
    isAvailable: true,
    placeholderSlot: "menu-vegetable-pizza.jpg",
    imageUrl: "https://images.unsplash.com/photo-1571407970349-bc81e7e96d47?auto=format&fit=crop&w=1200&q=85",
    description: "Crisp bell peppers, sweet corn, zucchini slices, ripe tomatoes, and herbs on savory crust.",
  },
  {
    id: "pizza-6",
    nameEn: "Kids pizza",
    nameAm: "የህፃናት ፒዛ",
    nameOr: "Pitsaa Daa'immanii",
    category: "pizza",
    price: 450,
    isAvailable: true,
    placeholderSlot: "menu-kids-pizza.jpg",
    imageUrl: "https://images.unsplash.com/photo-1513104890138-7c749659a591?auto=format&fit=crop&w=1200&q=85",
    description: "Mild, kid-friendly personal cheese pizza on a thin soft crust with sweet tomato sauce.",
  },

  // ==========================================
  // 4. BURGER (4 items)
  // ==========================================
  {
    id: "burger-1",
    nameEn: "Cheese burger",
    nameAm: "ቺዝ በርገር",
    nameOr: "Bargerii Chiizii",
    category: "burger",
    price: 620,
    isAvailable: true,
    placeholderSlot: "menu-cheese-burger.jpg",
    imageUrl: "https://images.unsplash.com/photo-1568901346375-23c9450c58cd?auto=format&fit=crop&w=1200&q=85",
    description: "Grilled prime beef patty draped with melted cheddar cheese, crisp lettuce, tomato, on a toasted sesame bun.",
  },
  {
    id: "burger-2",
    nameEn: "Chicken burger",
    nameAm: "የዶሮ በርገር",
    nameOr: "Bargerii Lukkuu",
    category: "burger",
    price: 800,
    isAvailable: true,
    placeholderSlot: "menu-chicken-burger.jpg",
    imageUrl: "https://images.unsplash.com/photo-1625813506062-0aeb1d7a094b?auto=format&fit=crop&w=1200&q=85",
    description: "Golden crispy fried chicken breast fillet with garlic mayo and shredded lettuce inside a brioche bun.",
  },
  {
    id: "burger-3",
    nameEn: "Normal burger",
    nameAm: "መደበኛ በርገር",
    nameOr: "Bargerii Idilee",
    category: "burger",
    price: 620,
    isAvailable: true,
    placeholderSlot: "menu-normal-burger.jpg",
    imageUrl: "https://images.unsplash.com/photo-1550547660-d9450f859349?auto=format&fit=crop&w=1200&q=85",
    description: "Pure lean ground beef patty flame-grilled with fresh sliced pickles, sweet onions, and house burger relish.",
  },
  {
    id: "burger-4",
    nameEn: "Special Bisrat burger",
    nameAm: "ልዩ ብስራት በርገር",
    nameOr: "Bargerii Addaa Bisrat",
    category: "burger",
    price: 850,
    isAvailable: true,
    placeholderSlot: "menu-special-bisrat-burger.jpg",
    imageUrl: "https://images.unsplash.com/photo-1586190848861-99aa4a171e90?auto=format&fit=crop&w=1200&q=85",
    description: "Grand double-stack beef burger with caramelized onions, fried egg, cheese slice, and secret house sauce.",
  },

  // ==========================================
  // 5. CHICKEN CORNER (8 items)
  // ==========================================
  {
    id: "chicken-1",
    nameEn: "Chicken Mexican",
    nameAm: "ቺከን ሜክሲካን",
    nameOr: "Lukkuu Meeksikoo",
    category: "chicken",
    price: 600,
    isAvailable: true,
    placeholderSlot: "menu-chicken-mexican.jpg",
    imageUrl: "/images/04_Spicy_Pan_Fried_Chicken.jpg",
    description: "Spicy Mexican-style shredded chicken seasoned with roasted peppers, cumin, and fresh cilantro garnish.",
  },
  {
    id: "chicken-2",
    nameEn: "Chicken nugget crispy",
    nameAm: "የዶሮ ነጌት (ክሪስፒ)",
    nameOr: "Lukkuu Nageetii Qaphxii",
    category: "chicken",
    price: 700,
    isAvailable: true,
    placeholderSlot: "menu-chicken-nugget.jpg",
    imageUrl: "https://images.unsplash.com/photo-1562967914-608f82629710?auto=format&fit=crop&w=1200&q=85",
    description: "Crispy panko-breaded tender chicken breast bites served with dipping sauces on a clean white plate.",
  },
  {
    id: "chicken-3",
    nameEn: "Chicken wing",
    nameAm: "የዶሮ ክንፍ ጥብስ",
    nameOr: "Koochoo Lukkuu",
    category: "chicken",
    price: 950,
    isAvailable: true,
    placeholderSlot: "menu-chicken-wing.jpg",
    imageUrl: "https://images.unsplash.com/photo-1567620832903-9fc6debc209f?auto=format&fit=crop&w=1200&q=85",
    description: "Tender glazed chicken wings baked and pan-finished with house barbecue spice rub.",
  },
  {
    id: "chicken-4",
    nameEn: "Grilled chicken",
    nameAm: "የተጠበሰ ዶሮ (ግሪልድ)",
    nameOr: "Lukkuu Dhaabame",
    category: "chicken",
    price: 950,
    isAvailable: true,
    placeholderSlot: "menu-grilled-chicken.jpg",
    imageUrl: "https://images.unsplash.com/photo-1598515214211-89d3c73ae83b?auto=format&fit=crop&w=1200&q=85",
    description: "Juicy marinated chicken fillet flame-grilled with distinct char marks and herbs.",
  },
  {
    id: "chicken-5",
    nameEn: "Lemon chicken",
    nameAm: "የሎሚ ዶሮ",
    nameOr: "Lukkuu Lomiitiin",
    category: "chicken",
    price: 650,
    isAvailable: true,
    placeholderSlot: "menu-lemon-chicken.jpg",
    imageUrl: "https://images.unsplash.com/photo-1532550907401-a500c9a57435?auto=format&fit=crop&w=1200&q=85",
    description: "Pan-seared tender chicken breast in a vibrant garlic lemon butter sauce with fresh thyme.",
  },
  {
    id: "chicken-6",
    nameEn: "Special Bisrat chicken",
    nameAm: "ልዩ የብስራት ዶሮ",
    nameOr: "Lukkuu Addaa Bisrat",
    category: "chicken",
    price: 1350,
    isAvailable: true,
    placeholderSlot: "menu-special-bisrat-chicken.jpg",
    imageUrl: "https://images.unsplash.com/photo-1626082927389-6cd097cdc6ec?auto=format&fit=crop&w=1200&q=85",
    description: "Chef's secret spice marinated chicken roasted to golden perfection and plated with sautéed vegetables.",
  },
  {
    id: "chicken-7",
    nameEn: "Half roast chicken",
    nameAm: "ግማሽ የተጠበሰ ሙሉ ዶሮ",
    nameOr: "Lukkuu Walakkaa Dhaabame",
    category: "chicken",
    price: 2550,
    isAvailable: true,
    placeholderSlot: "menu-half-roast-chicken.jpg",
    imageUrl: "/images/chicken-half-roast.jpg",
    description: "Succulent half roast chicken with crispy golden-brown skin, herb butter, and roasted potatoes.",
  },
  {
    id: "chicken-8",
    nameEn: "Whole roasted chicken",
    nameAm: "ሙሉ የተጠበሰ ዶሮ",
    nameOr: "Lukkuu Guutuu Dhaabame",
    category: "chicken",
    price: 5970,
    isAvailable: true,
    placeholderSlot: "menu-whole-roasted-chicken.jpg",
    imageUrl: "/images/chicken-whole-roast.jpg",
    description: "Magnificent whole roasted golden chicken on a serving platter with rosemary and lemon slices.",
  },

  // ==========================================
  // 6. PASTA (16 items)
  // ==========================================
  {
    id: "pasta-1",
    nameEn: "Chicken noodle",
    nameAm: "የዶሮ ኑድል",
    nameOr: "Nuudilii Lukkuu",
    category: "pasta",
    price: 700,
    isAvailable: true,
    placeholderSlot: "menu-chicken-noodle.jpg",
    imageUrl: "https://images.unsplash.com/photo-1552611052-33e04de081de?auto=format&fit=crop&w=1200&q=85",
    description: "Stir-fried noodles with chicken strips, crisp vegetables, and savory Asian garlic sauce.",
  },
  {
    id: "pasta-2",
    nameEn: "Pasta with Arbiyata",
    nameAm: "ፓስታ ከአረቢያታ ጋር",
    nameOr: "Paastaa Arabiyaataa Wajjin",
    category: "pasta",
    price: 420,
    isAvailable: true,
    placeholderSlot: "menu-penne-with-arbiyata.png",
    imageUrl: "/images/menu-penne-with-arbiyata.png",
    description: "Pasta in fiery garlic chili tomato sauce, garnished with fresh basil leaves.",
  },
  {
    id: "pasta-3",
    nameEn: "Pasta with Bolognese",
    nameAm: "ፓስታ ከቦሎኔዝ ጋር",
    nameOr: "Paastaa Boloneezii Wajjin",
    category: "pasta",
    price: 550,
    isAvailable: true,
    placeholderSlot: "menu-pasta-bolonez.jpg",
    imageUrl: "/images/pasta-bolognese.jpg",
    description: "Classic spaghetti bolognese tossed in a rich slow-simmered minced beef tomato ragù and parmesan.",
  },
  {
    id: "pasta-4",
    nameEn: "Pasta with Carbonara",
    nameAm: "ፓስታ ከካርቦናራ ጋር",
    nameOr: "Paastaa Kaarbonaaraa Wajjin",
    category: "pasta",
    price: 425,
    isAvailable: true,
    placeholderSlot: "menu-pasta-with-carbonara.png",
    imageUrl: "/images/menu-pasta-with-carbonara.png",
    description: "Al dente pasta swirled in creamy parmesan egg sauce with cured meat crisps.",
  },
  {
    id: "pasta-5",
    nameEn: "Pasta with Tomato",
    nameAm: "ፓስታ ከቲማቲም ጋር",
    nameOr: "Paastaa Timaatima Wajjin",
    category: "pasta",
    price: 380,
    isAvailable: true,
    placeholderSlot: "menu-pasta-with-tomato.jpg",
    imageUrl: "/images/menu-pasta-with-tomato.jpg",
    description: "Simple Italian comfort: spaghetti in freshly crushed tomato and olive oil pomodoro sauce.",
  },
  {
    id: "pasta-6",
    nameEn: "Pasta with Egg",
    nameAm: "ፓስታ ከእንቁላል ጋር",
    nameOr: "Paastaa Hanqaaquu Wajjin",
    category: "pasta",
    price: 410,
    isAvailable: true,
    placeholderSlot: "menu-pasta-egg.jpg",
    imageUrl: "https://images.unsplash.com/photo-1546549032-9571cd6b27df?auto=format&fit=crop&w=1200&q=85",
    description: "Sautéed pasta crowned with a sunny-side-up farm fresh egg and ground black pepper.",
  },
  {
    id: "pasta-7",
    nameEn: "Pasta with Tuna",
    nameAm: "ፓስታ ከቱና ጋር",
    nameOr: "Paastaa Tuunaa Wajjin",
    category: "pasta",
    price: 485,
    isAvailable: true,
    placeholderSlot: "menu-pasta-tuna.jpg",
    imageUrl: "https://images.unsplash.com/photo-1608897013039-887f21d8c804?auto=format&fit=crop&w=1200&q=85",
    description: "Spaghetti tossed with ocean tuna flakes, garlic, cherry tomatoes, and parsley.",
  },
  {
    id: "pasta-8",
    nameEn: "Pasta with Vegetable",
    nameAm: "ፓስታ ከአትክልት ጋር",
    nameOr: "Paastaa Kuduraa Wajjin",
    category: "pasta",
    price: 370,
    isAvailable: true,
    placeholderSlot: "menu-pasta-vegetable.jpg",
    imageUrl: "https://images.unsplash.com/photo-1473093295043-cdd812d0e601?auto=format&fit=crop&w=1200&q=85",
    description: "Pasta tossed with colorful garden vegetables including zucchini, carrots, and sweet bell peppers.",
  },
  {
    id: "pasta-9",
    nameEn: "Telateli",
    nameAm: "ታሊያተሊ ፓስታ",
    nameOr: "Paastaa Teliyaatelii",
    category: "pasta",
    price: 390,
    isAvailable: true,
    placeholderSlot: "menu-telateli.jpg",
    imageUrl: "https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?auto=format&fit=crop&w=1200&q=85",
    description: "Delicate flat ribbon tagliatelle pasta coated with savory tomato herb reduction.",
  },
  {
    id: "pasta-10",
    nameEn: "Penne with Bolognese",
    nameAm: "ማካሮኒ ከቦሎኔዝ ጋር",
    nameOr: "Maakaroonii Boloneezii Wajjin",
    category: "pasta",
    price: 465,
    isAvailable: true,
    placeholderSlot: "menu-penne-with-bolognese.png",
    imageUrl: "/images/menu-penne-with-bolognese.png",
    description: "Ridged penne tubes covered in hearty slow-simmered meat bolognese sauce.",
  },
  {
    id: "pasta-11",
    nameEn: "Penne with Tuna",
    nameAm: "ማካሮኒ ከቱና ጋር",
    nameOr: "Maakaroonii Tuunaa Wajjin",
    category: "pasta",
    price: 485,
    isAvailable: true,
    placeholderSlot: "menu-penne-with-tuna.png",
    imageUrl: "/images/menu-penne-with-tuna.png",
    description: "Al dente penne paired with savory flaked tuna and rich tomato basil sauce.",
  },
  {
    id: "pasta-12",
    nameEn: "Penne with Carbonara",
    nameAm: "ማካሮኒ ከካርቦናራ ጋር",
    nameOr: "Maakaroonii Kaarbonaaraa Wajjin",
    category: "pasta",
    price: 425,
    isAvailable: true,
    placeholderSlot: "menu-pasta-with-carbonara.png",
    imageUrl: "/images/menu-pasta-with-carbonara.png",
    description: "Creamy silky carbonara reduction coating warm al dente penne pasta.",
  },
  {
    id: "pasta-13",
    nameEn: "Penne with Tomato",
    nameAm: "ማካሮኒ ከቲማቲም ጋር",
    nameOr: "Maakaroonii Timaatima Wajjin",
    category: "pasta",
    price: 380,
    isAvailable: true,
    placeholderSlot: "menu-penne-with-tomato.png",
    imageUrl: "/images/menu-penne-with-tomato.png",
    description: "Tender penne finished in fragrant home-style Italian tomato sauce.",
  },
  {
    id: "pasta-14",
    nameEn: "Penne with Vegetable",
    nameAm: "ማካሮኒ ከአትክልት ጋር",
    nameOr: "Maakaroonii Kuduraa Wajjin",
    category: "pasta",
    price: 370,
    isAvailable: true,
    placeholderSlot: "menu-penne-with-vegetable.png",
    imageUrl: "/images/menu-penne-with-vegetable.png",
    description: "Vibrant vegetable medley pan-sautéed with penne pasta.",
  },
  {
    id: "pasta-15",
    nameEn: "Penne with Arbiyata",
    nameAm: "ማካሮኒ ከአረቢያታ ጋር",
    nameOr: "Maakaroonii Arabiyaataa Wajjin",
    category: "pasta",
    price: 420,
    isAvailable: true,
    placeholderSlot: "menu-penne-with-arbiyata.png",
    imageUrl: "/images/menu-penne-with-arbiyata.png",
    description: "Fiery arrabbiata tomato chili sauce tossed over al dente penne pasta.",
  },
  {
    id: "pasta-16",
    nameEn: "Penne with Egg",
    nameAm: "ማካሮኒ ከእንቁላል ጋር",
    nameOr: "Maakaroonii Hanqaaquu Wajjin",
    category: "pasta",
    price: 410,
    isAvailable: true,
    placeholderSlot: "menu-penne-with-egg.png",
    imageUrl: "/images/menu-penne-with-egg.png",
    description: "Warm penne pasta accompanied by a freshly prepared farm egg.",
  },

  // ==========================================
  // 7. TRADITIONAL CORNER (11 items)
  // ==========================================
  {
    id: "trad-1",
    nameEn: "Bozena shiro",
    nameAm: "ቦዘና ሽሮ",
    nameOr: "Bozanaa Shiroo",
    category: "traditional",
    price: 380,
    isAvailable: true,
    placeholderSlot: "menu-bozena-shiro.jpg",
    imageUrl: "/images/ethiopian-shiro.jpg",
    description: "Traditional roasted chickpea stew simmered with tender minced beef and spiced butter, served with fresh injera.",
  },
  {
    id: "trad-2",
    nameEn: "Chekena tibs",
    nameAm: "ጨከና ጥብስ",
    nameOr: "Cakanaa Tibsii",
    category: "traditional",
    price: 650,
    isAvailable: true,
    placeholderSlot: "menu-chekena-tibs.jpg",
    imageUrl: "/images/01_Awaze_Tibs_with_Bread_and_Injera.jpg",
    description: "Finest prime tenderloin beef cubes sautéed with rosemary, red onions, garlic, and fresh jalapeños.",
  },
  {
    id: "trad-3",
    nameEn: "Fasting shiro",
    nameAm: "የጾም ሽሮ",
    nameOr: "Shiroo Soomaa",
    category: "traditional",
    price: 320,
    isAvailable: true,
    placeholderSlot: "menu-fasting-shiro.jpg",
    imageUrl: "/images/06_Shiro_with_Injera_Rolls.jpg",
    description: "Silky spiced chickpea and split-pea stew cooked with garlic and berbere, 100% vegan with injera.",
  },
  {
    id: "trad-4",
    nameEn: "Lamb tibs",
    nameAm: "የበግ ጥብስ",
    nameOr: "Tibsii Hoolaa",
    category: "traditional",
    price: 720,
    isAvailable: true,
    placeholderSlot: "menu-lamb-tibs.jpg",
    imageUrl: "/images/19_Beef_Tibs_with_Injera.jpg",
    description: "Succulent cubes of tender highland lamb seared in spiced butter with peppers and onions.",
  },
  {
    id: "trad-5",
    nameEn: "Shiro with butter",
    nameAm: "ሽሮ በቅቤ",
    nameOr: "Shiroo Dhadhaan",
    category: "traditional",
    price: 380,
    isAvailable: true,
    placeholderSlot: "menu-shiro-butter.jpg",
    imageUrl: "/images/14_Shiro_with_Injera_Plate.jpg",
    description: "Steaming hot chickpea stew finished with a lavish spoonful of fragrant Ethiopian niter kibbeh.",
  },
  {
    id: "trad-6",
    nameEn: "Special shiro",
    nameAm: "ልዩ ሽሮ",
    nameOr: "Shiroo Addaa",
    category: "traditional",
    price: 420,
    isAvailable: true,
    placeholderSlot: "menu-special-shiro.jpg",
    imageUrl: "/images/16_Shiro_Dip_Bowl.jpg",
    description: "House signature shiro loaded with extra aromatics, butter, and served bubbling with fresh injera.",
  },
  {
    id: "trad-7",
    nameEn: "Suf fitfit",
    nameAm: "ሱፍ ፍትፍት",
    nameOr: "Suufi Fitfit",
    category: "traditional",
    price: 450,
    isAvailable: true,
    placeholderSlot: "menu-suf-fitfit.png",
    imageUrl: "/images/menu-suf-fitfit.png",
    description: "Shredded fresh injera steeped in chilled savory ground niger-seed milk, peppers, and onions.",
  },
  {
    id: "trad-8",
    nameEn: "Tegabino",
    nameAm: "ተጋቢኖ ሽሮ",
    nameOr: "Tagaabinoo",
    category: "traditional",
    price: 350,
    isAvailable: true,
    placeholderSlot: "menu-tegabino.jpg",
    imageUrl: "/images/20_Shiro_with_Injera.jpg",
    description: "Thick, bubbling chickpea stew served sizzling directly in a traditional Ethiopian earthenware clay pot.",
  },
  {
    id: "trad-9",
    nameEn: "Dulet",
    nameAm: "ዱለት",
    nameOr: "Duleetii",
    category: "traditional",
    price: 450,
    isAvailable: true,
    placeholderSlot: "menu-dulet.jpg",
    imageUrl: "/images/menu-dulet.jpg",
    description: "Finely minced tripe, liver, and lean beef seasoned with mitmita, niter kibbeh, and green chili.",
  },
  {
    id: "trad-10",
    nameEn: "Bisrat fasting combo",
    nameAm: "የብስራት ልዩ የጾም በያይነቱ",
    nameOr: "Beyaynetuu Soomaa Bisrat",
    category: "traditional",
    price: 1780,
    isAvailable: true,
    placeholderSlot: "menu-bisrat-fasting-combo.png",
    imageUrl: "/images/menu-bisrat-fasting-combo.png",
    description: "Platter of assorted vegan delicacies: Misir wot, Kik alicha, Gomen, Atakilt wot, Shiro, and Fosolia on injera.",
  },
  {
    id: "trad-11",
    nameEn: "Bisrat special combo",
    nameAm: "የብስራት ልዩ የሥጋ በያይነቱ",
    nameOr: "Beyaynetuu Foonii Bisrat",
    category: "traditional",
    price: 4655,
    isAvailable: true,
    placeholderSlot: "menu-bisrat-special-combo.png",
    imageUrl: "/images/menu-bisrat-special-combo.png",
    description: "Grand banquet feast: Special kitfo, doro wot, sizzling beef tibs, gomen be-sega, and assorted sides on giant injera.",
  },

  // ==========================================
  // 8. SOUP CORNER (9 items)
  // ==========================================
  {
    id: "soup-1",
    nameEn: "Beet root soup",
    nameAm: "የቀይ ስር ሾርባ",
    nameOr: "Shorbaa Qayyisiraa",
    category: "soup",
    price: 200,
    isAvailable: true,
    placeholderSlot: "menu-beet-root-soup.jpg",
    imageUrl: "/images/10_Beet_Soup.webp",
    description: "Vibrant ruby-magenta beetroot soup lightly seasoned and swirled with fresh cream.",
  },
  {
    id: "soup-2",
    nameEn: "Chicken cream soup",
    nameAm: "የዶሮ ክሬም ሾርባ",
    nameOr: "Shorbaa Kiriimii Lukkuu",
    category: "soup",
    price: 300,
    isAvailable: true,
    placeholderSlot: "menu-chicken-cream-soup.jpg",
    imageUrl: "https://images.unsplash.com/photo-1547592180-85f173990554?auto=format&fit=crop&w=1200&q=85",
    description: "Velvety smooth chicken broth with shredded tender breast meat, cream, and freshly chopped herbs.",
  },
  {
    id: "soup-3",
    nameEn: "Hungarian soup",
    nameAm: "የሃንጋሪ ሾርባ (ጉላሽ)",
    nameOr: "Shorbaa Hangarii",
    category: "soup",
    price: 300,
    isAvailable: true,
    placeholderSlot: "menu-hungarian-soup.jpg",
    imageUrl: "https://images.unsplash.com/photo-1541832676-9b763b0239ab?auto=format&fit=crop&w=1200&q=85",
    description: "Hearty reddish beef goulash soup with potatoes, paprika, and savory root vegetables.",
  },
  {
    id: "soup-4",
    nameEn: "Minestrone soup",
    nameAm: "የአታክልት ሚኒስትሮኔ ሾርባ",
    nameOr: "Shorbaa Minestroonii",
    category: "soup",
    price: 200,
    isAvailable: true,
    placeholderSlot: "menu-minestrone-soup.jpg",
    imageUrl: "https://images.unsplash.com/photo-1603105037880-880cd4edfb0d?auto=format&fit=crop&w=1200&q=85",
    description: "Traditional Italian chunky vegetable soup with beans, tomatoes, celery, and ditalini pasta.",
  },
  {
    id: "soup-5",
    nameEn: "Mushroom cream soup",
    nameAm: "የእንጉዳይ ክሬም ሾርባ",
    nameOr: "Shorbaa Marqaa Boombii",
    category: "soup",
    price: 300,
    isAvailable: true,
    placeholderSlot: "menu-mushroom-cream-soup.jpg",
    imageUrl: "https://images.unsplash.com/photo-1547592180-85f173990554?auto=format&fit=crop&w=1200&q=85",
    description: "Rich earth-mushroom velouté enriched with cream, garlic, and cracked pepper.",
  },
  {
    id: "soup-6",
    nameEn: "Special Bisrat soup",
    nameAm: "ልዩ የብስራት ሾርባ",
    nameOr: "Shorbaa Addaa Bisrat",
    category: "soup",
    price: 250,
    isAvailable: true,
    placeholderSlot: "menu-special-bisrat-soup.jpg",
    imageUrl: "https://images.unsplash.com/photo-1582878826629-29b7ad1cdc43?auto=format&fit=crop&w=1200&q=85",
    description: "House chef's special warming blend of chicken, select vegetables, and Ethiopian aromatic spices.",
  },
  {
    id: "soup-7",
    nameEn: "Tomato soup",
    nameAm: "የቲማቲም ሾርባ",
    nameOr: "Shorbaa Timaatimaa",
    category: "soup",
    price: 210,
    isAvailable: true,
    placeholderSlot: "menu-tomato-soup.jpg",
    imageUrl: "/images/02_Tomato_Soup_with_Basil.jpg",
    description: "Sweet sun-ripened roasted tomato soup topped with cream swirl and fresh basil.",
  },
  {
    id: "soup-8",
    nameEn: "Vegetable soup",
    nameAm: "የተደባለቀ አታክልት ሾርባ",
    nameOr: "Shorbaa Kuduraa",
    category: "soup",
    price: 200,
    isAvailable: true,
    placeholderSlot: "menu-vegetable-soup.jpg",
    imageUrl: "https://images.unsplash.com/photo-1603105037880-880cd4edfb0d?auto=format&fit=crop&w=1200&q=85",
    description: "Clear garden broth loaded with freshly diced zucchini, carrots, potatoes, and parsley.",
  },
  {
    id: "soup-9",
    nameEn: "Carrot soup",
    nameAm: "የካሮት ሾርባ",
    nameOr: "Shorbaa Kaarotii",
    category: "soup",
    price: 200,
    isAvailable: true,
    placeholderSlot: "menu-carrot-soup.jpg",
    imageUrl: "/images/03_Carrot_Soup_with_Cream_Swirl.jpg",
    description: "Smooth golden carrot soup simmered with ginger, coriander, and light coconut cream.",
  },

  // ==========================================
  // 9. SALAD CORNER (5 items)
  // ==========================================
  {
    id: "salad-1",
    nameEn: "Avocado salad",
    nameAm: "የአቮካዶ ሰላጣ",
    nameOr: "Salaaxaa Avokaadoo",
    category: "salad",
    price: 300,
    isAvailable: true,
    placeholderSlot: "menu-avocado-salad.jpg",
    imageUrl: "/images/01_Avocado_Tomato_Cucumber_Salad.jpg",
    description: "Ripe butter avocado slices laid over crisp mixed greens, tomatoes, red onion, and lime vinaigrette.",
  },
  {
    id: "salad-2",
    nameEn: "Chicken salad",
    nameAm: "የዶሮ ሰላጣ",
    nameOr: "Salaaxaa Lukkuu",
    category: "salad",
    price: 450,
    isAvailable: true,
    placeholderSlot: "menu-chicken-salad.jpg",
    imageUrl: "/images/05_Grilled_Chicken_Avocado_Corn_Salad.jpg",
    description: "Tender grilled chicken slices atop crunchy romaine lettuce, cucumber, and light house dressing.",
  },
  {
    id: "salad-3",
    nameEn: "Mixed salad",
    nameAm: "የተደባለቀ ሰላጣ",
    nameOr: "Salaaxaa Walmakaa",
    category: "salad",
    price: 300,
    isAvailable: true,
    placeholderSlot: "menu-mixed-salad.jpg",
    imageUrl: "/images/07_Mixed_Garden_Salad.jpg",
    description: "Fresh crisp mixed garden salad with crunchy greens, cucumber, ripe tomatoes, and light vinaigrette.",
  },
  {
    id: "salad-4",
    nameEn: "Roasted salad",
    nameAm: "የተጠበሰ አታክልት ሰላጣ",
    nameOr: "Salaaxaa Kuduraa Dhaabame",
    category: "salad",
    price: 250,
    isAvailable: true,
    placeholderSlot: "menu-roasted-salad.jpg",
    imageUrl: "/images/03_Roasted_Butternut_Squash_Quinoa_Salad.jpg",
    description: "Oven-roasted peppers, eggplants, and sweet onions tossed in balsamic vinaigrette.",
  },
  {
    id: "salad-5",
    nameEn: "Tuna salad",
    nameAm: "የቱና ዓሣ ሰላጣ",
    nameOr: "Salaaxaa Tuunaa",
    category: "salad",
    price: 520,
    isAvailable: true,
    placeholderSlot: "menu-tuna-salad.jpg",
    imageUrl: "/images/04_Tuna_Apple_Salad.jpg",
    description: "Flaked ocean tuna over crisp garden leaves, sliced boiled eggs, olives, and fresh tomatoes.",
  },

  // ==========================================
  // 10. FISH CORNER (5 items)
  // ==========================================
  {
    id: "fish-1",
    nameEn: "Fish cotelet",
    nameAm: "የዓሣ ኩትሌት",
    nameOr: "Qutleeti Qurxummii",
    category: "fish",
    price: 500,
    isAvailable: true,
    placeholderSlot: "menu-fish-cotelet.jpg",
    imageUrl: "/images/09_Breaded_Cutlets_with_Fries.jpg",
    description: "Golden breaded fried lake fish cutlet served with zesty tartar sauce and fresh lemon wedges.",
  },
  {
    id: "fish-2",
    nameEn: "Fish and chips",
    nameAm: "ዓሣ ከቺፕስ ጋር",
    nameOr: "Qurxummii fi Chips",
    category: "fish",
    price: 650,
    isAvailable: true,
    placeholderSlot: "menu-fish-chips.jpg",
    imageUrl: "/images/14_Crispy_Whole_Fish_with_Pepper_Sauce.jpg",
    description: "Crispy battered fish fillets accompanied by a generous portion of seasoned french fries.",
  },
  {
    id: "fish-3",
    nameEn: "Fish gulash",
    nameAm: "የዓሣ ጉላሽ",
    nameOr: "Gulaashii Qurxummii",
    category: "fish",
    price: 570,
    isAvailable: true,
    placeholderSlot: "menu-fish-gulash.jpg",
    imageUrl: "https://images.unsplash.com/photo-1534939561126-855b8675edd7?auto=format&fit=crop&w=1200&q=85",
    description: "Tender chunks of lake fish simmered in a spiced tomato, paprika, and bell pepper stew.",
  },
  {
    id: "fish-4",
    nameEn: "Grilled fish",
    nameAm: "የተጠበሰ ሙሉ ዓሣ",
    nameOr: "Qurxummii Dhaabame",
    category: "fish",
    price: 585,
    isAvailable: true,
    placeholderSlot: "menu-grilled-fish.jpg",
    imageUrl: "/images/11_Whole_Spicy_Fried_Fish_with_Salsa.jpg",
    description: "Whole fresh lake fish seasoned with herbs and flame-grilled, accompanied by lemon slices and steamed veggies.",
  },
  {
    id: "fish-5",
    nameEn: "Stir fried fish",
    nameAm: "የተጠበሰ ቁርጥራጭ ዓሣ ከአታክልት ጋር",
    nameOr: "Qurxummii Qodamaa Kuduraan",
    category: "fish",
    price: 620,
    isAvailable: true,
    placeholderSlot: "menu-stir-fried-fish.jpg",
    imageUrl: "https://images.unsplash.com/photo-1534422298391-e4f8c172dddb?auto=format&fit=crop&w=1200&q=85",
    description: "Tender fish bites wok-fried with crisp bell peppers, onions, ginger, and light soy glaze.",
  },

  // ==========================================
  // 11. BREAKFAST (15 items)
  // ==========================================
  {
    id: "bf-1",
    nameEn: "Chechebsa",
    nameAm: "ጨጨብሳ",
    nameOr: "Caccabsaa",
    category: "breakfast",
    price: 320,
    isAvailable: true,
    placeholderSlot: "menu-chechebsa.jpg",
    imageUrl: "/images/ethiopian-chechebsa.jpg",
    description: "Ethiopian shredded pita flatbread pan-tossed in fragrant niter kibbeh spiced butter and berbere.",
  },
  {
    id: "bf-2",
    nameEn: "Cheese omelet",
    nameAm: "ቺዝ ኦምሌት",
    nameOr: "Omleetii Chiizii",
    category: "breakfast",
    price: 350,
    isAvailable: true,
    placeholderSlot: "menu-cheese-omelet.jpg",
    imageUrl: "https://images.unsplash.com/photo-1510693206972-df098062cb71?auto=format&fit=crop&w=1200&q=85",
    description: "Fluffy folded golden egg omelet bursting with melted cheddar and mozzarella cheese.",
  },
  {
    id: "bf-3",
    nameEn: "Continental breakfast",
    nameAm: "ኮንቲኔንታል ቁርስ",
    nameOr: "Ciree Kontineentaalii",
    category: "breakfast",
    price: 350,
    isAvailable: true,
    placeholderSlot: "menu-continental-bf.jpg",
    imageUrl: "https://images.unsplash.com/photo-1533089860892-a7c6f0a88666?auto=format&fit=crop&w=1200&q=85",
    description: "Toasted breads, butter, marmalade, fresh eggs, and sliced fruits served with morning tea or coffee.",
  },
  {
    id: "bf-4",
    nameEn: "Fasting firfir",
    nameAm: "የጾም ፍርፍር",
    nameOr: "Firfir Soomaa",
    category: "breakfast",
    price: 200,
    isAvailable: true,
    placeholderSlot: "menu-fasting-firfir.jpg",
    imageUrl: "/images/ethiopian-firfir.jpg",
    description: "Shredded fresh injera cooked in a savory, aromatic berbere, tomato, and onion sauce.",
  },
  {
    id: "bf-5",
    nameEn: "Kinche",
    nameAm: "ኪንጬ",
    nameOr: "Qincee",
    category: "breakfast",
    price: 200,
    isAvailable: true,
    placeholderSlot: "menu-kinche.jpg",
    imageUrl: "/images/menu-kinche.jpg",
    description: "Traditional cracked wheat porridge gently simmered and flavored with Ethiopian spiced butter or oil.",
  },
  {
    id: "bf-6",
    nameEn: "Oats",
    nameAm: "ኦትስ (አጃ ሾርባ)",
    nameOr: "Ootsii",
    category: "breakfast",
    price: 250,
    isAvailable: true,
    placeholderSlot: "menu-oats.jpg",
    imageUrl: "/images/06_Spiced_Oatmeal_Porridge.jpg",
    description: "Warm wholesome oatmeal cooked with fresh milk, honey drizzle, and cinnamon dusting.",
  },
  {
    id: "bf-7",
    nameEn: "Omelet",
    nameAm: "ኦምሌት",
    nameOr: "Omleetii",
    category: "breakfast",
    price: 250,
    isAvailable: true,
    placeholderSlot: "menu-omelet.jpg",
    imageUrl: "/images/02_Vegetable_Omelette.jpg",
    description: "Classic pan-fried egg omelet with finely chopped sweet onions and green peppers.",
  },
  {
    id: "bf-8",
    nameEn: "Pancake",
    nameAm: "ፓንኬክ",
    nameOr: "Paankekii",
    category: "breakfast",
    price: 200,
    isAvailable: true,
    placeholderSlot: "menu-pancake.jpg",
    imageUrl: "https://images.unsplash.com/photo-1528207776546-365bb710ee93?auto=format&fit=crop&w=1200&q=85",
    description: "Stack of fluffy golden pancakes topped with butter pat and rich pure honey.",
  },
  {
    id: "bf-9",
    nameEn: "Quanta firfir",
    nameAm: "ቋንጣ ፍርፍር",
    nameOr: "Firfir Qwantaa",
    category: "breakfast",
    price: 450,
    isAvailable: true,
    placeholderSlot: "menu-quanta-firfir.jpg",
    imageUrl: "/images/ethiopian-firfir.jpg",
    description: "Dried cured spiced beef (quanta) stewed in rich berbere sauce and infused with rolled injera pieces.",
  },
  {
    id: "bf-10",
    nameEn: "Scramble egg",
    nameAm: "የተፈጨ እንቁላል ጥብስ",
    nameOr: "Hanqaaquu Boorame",
    category: "breakfast",
    price: 250,
    isAvailable: true,
    placeholderSlot: "menu-scramble-egg.jpg",
    imageUrl: "/images/17_Beef_and_Egg_Scramble.jpg",
    description: "Freshly scrambled farm eggs seasoned with sea salt, butter, and black pepper.",
  },
  {
    id: "bf-11",
    nameEn: "Scramble egg with meat",
    nameAm: "እንቁላል ጥብስ በሥጋ",
    nameOr: "Hanqaaquu Fooniin",
    category: "breakfast",
    price: 300,
    isAvailable: true,
    placeholderSlot: "menu-scramble-egg-meat.jpg",
    imageUrl: "/images/12_Minced_Meat_with_Injera_and_Eggs.jpg",
    description: "Hearty scrambled eggs sautéed with spiced minced beef, onions, and jalapeño pepper.",
  },
  {
    id: "bf-12",
    nameEn: "Sega firfir",
    nameAm: "የሥጋ ፍርፍር",
    nameOr: "Firfir Foonii",
    category: "breakfast",
    price: 400,
    isAvailable: true,
    placeholderSlot: "menu-sega-firfir.jpg",
    imageUrl: "/images/ethiopian-firfir.jpg",
    description: "Tender sautéed beef pieces folded into spicy injera firfir in rich tomato sauce.",
  },
  {
    id: "bf-13",
    nameEn: "Tebs firfir",
    nameAm: "የጥብስ ፍርፍር",
    nameOr: "Firfir Tibsii",
    category: "breakfast",
    price: 450,
    isAvailable: true,
    placeholderSlot: "menu-tebs-firfir.jpg",
    imageUrl: "/images/13_Beef_Tibs_with_Injera_and_Wine.jpg",
    description: "Pan-sautéed succulent beef tibs combined with seasoned injera firfir and green chilies.",
  },
  {
    id: "bf-14",
    nameEn: "Dulet (breakfast)",
    nameAm: "የቁርስ ዱለት",
    nameOr: "Duleetii Ciree",
    category: "breakfast",
    price: 450,
    isAvailable: true,
    placeholderSlot: "menu-dulet-bf.jpg",
    imageUrl: "/images/menu-dulet.jpg",
    description: "Morning specialty dulet: minced liver, tripe, and lean meat spiced with mitmita and served with hot injera.",
  },
  {
    id: "bf-15",
    nameEn: "Fetira with honey",
    nameAm: "ፈቲራ በማር",
    nameOr: "Fatiiraa Dammaan",
    category: "breakfast",
    price: 300,
    isAvailable: true,
    placeholderSlot: "menu-fetira-honey.jpg",
    imageUrl: "/images/07_Stuffed_Flatbread_with_Dipping_Sauce.jpg",
    description: "Flaky layered Ethiopian pancake pastry fried crisp and generously drizzled with pure highland honey.",
  },

  // ==========================================
  // 12. JUICE CORNER (11 items)
  // ==========================================
  {
    id: "juice-1",
    nameEn: "Avocado juice",
    nameAm: "የአቮካዶ ጁስ",
    nameOr: "Juusii Avokaadoo",
    category: "juice",
    price: 150,
    isAvailable: true,
    placeholderSlot: "menu-avocado-juice.jpg",
    imageUrl: "/images/10_Avocado_Smoothie.jpg",
    description: "Thick, silky smooth Ethiopian-style avocado purée served in a tall glass with a squeeze of fresh lime.",
  },
  {
    id: "juice-2",
    nameEn: "Banana shake",
    nameAm: "የሙዝ ሼክ",
    nameOr: "Sheekii Muuzii",
    category: "juice",
    price: 200,
    isAvailable: true,
    placeholderSlot: "menu-banana-shake.jpg",
    imageUrl: "/images/juice-banana-shake.jpg",
    description: "Rich creamy pale yellow banana milkshake smoothie topped with fresh banana slices and a paper straw.",
  },
  {
    id: "juice-3",
    nameEn: "Fruit punch",
    nameAm: "የፍራፍሬ ፓንች",
    nameOr: "Fruut Paanchii",
    category: "juice",
    price: 200,
    isAvailable: true,
    placeholderSlot: "menu-fruit-punch.jpg",
    imageUrl: "/images/18_Fruit_Punch.jpg",
    description: "Vibrant tropical fruit punch cocktail made with fresh fruit extracts and citrus.",
  },
  {
    id: "juice-4",
    nameEn: "Mango juice",
    nameAm: "የማንጎ ጁስ",
    nameOr: "Juusii Maangoo",
    category: "juice",
    price: 150,
    isAvailable: true,
    placeholderSlot: "menu-mango-juice.jpg",
    imageUrl: "/images/juice-mango.jpg",
    description: "Pure rich golden mango juice pressed from fresh ripe mangoes, served chilled and sweet.",
  },
  {
    id: "juice-5",
    nameEn: "Mixed juice",
    nameAm: "የተደባለቀ (ስፕሪስ) ጁስ",
    nameOr: "Juusii Spris Walmakaa",
    category: "juice",
    price: 180,
    isAvailable: true,
    placeholderSlot: "menu-mixed-juice.jpg",
    imageUrl: "/images/juice-spris-mixed.jpg",
    description: "Authentic Ethiopian Spris (ስፕሪስ) with distinct colorful horizontal layers of avocado, mango, and papaya.",
  },
  {
    id: "juice-6",
    nameEn: "Papaya juice",
    nameAm: "የፓፓያ ጁስ",
    nameOr: "Juusii Paappayyaa",
    category: "juice",
    price: 150,
    isAvailable: true,
    placeholderSlot: "menu-papaya-juice.jpg",
    imageUrl: "/images/juice-papaya.jpg",
    description: "Fresh coral-orange papaya nectar served in a tall glass with a fresh lime wheel garnish and straw.",
  },
  {
    id: "juice-7",
    nameEn: "Pineapple juice",
    nameAm: "የአናናስ ጁስ",
    nameOr: "Juusii Anaanaasii",
    category: "juice",
    price: 180,
    isAvailable: true,
    placeholderSlot: "menu-pineapple-juice.jpg",
    imageUrl: "/images/juice-pineapple.jpg",
    description: "Fresh chilled golden pineapple juice served in a tall glass with ice cubes, straw, and a fresh pineapple slice.",
  },
  {
    id: "juice-8",
    nameEn: "Special juice",
    nameAm: "ልዩ የብስራት ጁስ",
    nameOr: "Juusii Addaa Bisrat",
    category: "juice",
    price: 250,
    isAvailable: true,
    placeholderSlot: "menu-special-juice.jpg",
    imageUrl: "/images/juice-spris-mixed.jpg",
    description: "Chef's signature multi-fruit layered Ethiopian special juice, richly blended and served ice cold.",
  },
  {
    id: "juice-9",
    nameEn: "Strawberry juice",
    nameAm: "የእንጆሪ ጁስ",
    nameOr: "Juusii Istirooberii",
    category: "juice",
    price: 230,
    isAvailable: true,
    placeholderSlot: "menu-strawberry-juice.jpg",
    imageUrl: "/images/juice-strawberry.jpg",
    description: "Fresh chilled red strawberry juice served in a glass with ripe berry notes.",
  },
  {
    id: "juice-10",
    nameEn: "Telba juice",
    nameAm: "የትልባ ጁስ",
    nameOr: "Juusii Talbaa",
    category: "juice",
    price: 180,
    isAvailable: true,
    placeholderSlot: "menu-telba-juice.jpg",
    imageUrl: "/images/juice-telba.jpg",
    description: "Traditional Ethiopian Telba (ተልባ) — roasted flaxseed smoothie gently sweetened and served chilled with ice.",
  },
  {
    id: "juice-11",
    nameEn: "Watermelon juice",
    nameAm: "የሐብሐብ ጁስ",
    nameOr: "Juusii Habhabii",
    category: "juice",
    price: 180,
    isAvailable: true,
    placeholderSlot: "menu-watermelon-juice.jpg",
    imageUrl: "/images/15_Watermelon_Mint_Cooler.jpg",
    description: "Crisp, cooling ruby-red watermelon juice served chilled with fresh mint garnish.",
  },

  // ==========================================
  // 13. HOT DRINK (8 items)
  // ==========================================
  {
    id: "hot-1",
    nameEn: "Machiato",
    nameAm: "ማኪያቶ",
    nameOr: "Makiyaatoo",
    category: "hot_drink",
    price: 70,
    isAvailable: true,
    placeholderSlot: "menu-machiato.jpg",
    imageUrl: "https://images.unsplash.com/photo-1514432324607-a09d9b4aefdd?auto=format&fit=crop&w=1200&q=85",
    description: "Classic Ethiopian macchiato with strong espresso layered under velvety frothed steamed milk.",
  },
  {
    id: "hot-2",
    nameEn: "Traditional coffee",
    nameAm: "የሀበሻ ባህላዊ ጀበና ቡና",
    nameOr: "Buna Aadaa Jabanaa",
    category: "hot_drink",
    price: 50,
    isAvailable: true,
    placeholderSlot: "menu-traditional-coffee.jpg",
    imageUrl: "/images/ethiopian-coffee.jpg",
    description: "Freshly roasted organic Ethiopian highland coffee brewed in a clay Jebena and poured into small cups.",
  },
  {
    id: "hot-3",
    nameEn: "Coffee/machine",
    nameAm: "የማሽን ቡና (ኤስፕሬሶ)",
    nameOr: "Buna Maashinii",
    category: "hot_drink",
    price: 70,
    isAvailable: true,
    placeholderSlot: "menu-machine-coffee.jpg",
    imageUrl: "https://images.unsplash.com/photo-1510591509098-f4fdc6d0ff04?auto=format&fit=crop&w=1200&q=85",
    description: "Rich and intense double-shot espresso brewed on commercial Italian espresso machines.",
  },
  {
    id: "hot-4",
    nameEn: "Milk",
    nameAm: "ትኩስ ወተት",
    nameOr: "Aannan Ho'aa",
    category: "hot_drink",
    price: 60,
    isAvailable: true,
    placeholderSlot: "menu-milk.jpg",
    imageUrl: "https://images.unsplash.com/photo-1550583724-b2692b85b150?auto=format&fit=crop&w=1200&q=85",
    description: "Steamed pure fresh whole milk served hot in a clear glass tumbler.",
  },
  {
    id: "hot-5",
    nameEn: "Peanut tea",
    nameAm: "የለውዝ ሻይ",
    nameOr: "Shaayee Lawzii",
    category: "hot_drink",
    price: 65,
    isAvailable: true,
    placeholderSlot: "menu-peanut-tea.jpg",
    imageUrl: "/images/hot-peanut-tea.jpg",
    description: "Sweet infused tea enriched with ground roasted peanuts for a rich, nutty warm beverage.",
  },
  {
    id: "hot-6",
    nameEn: "Special tea",
    nameAm: "ልዩ ቅመም ሻይ",
    nameOr: "Shaayee Qarafaa Addaa",
    category: "hot_drink",
    price: 80,
    isAvailable: true,
    placeholderSlot: "menu-special-tea.jpg",
    imageUrl: "https://images.unsplash.com/photo-1597481499750-3e6b22637e12?auto=format&fit=crop&w=1200&q=85",
    description: "Aromatic blend of black tea spiced with cinnamon, cardamom, cloves, and ginger.",
  },
  {
    id: "hot-7",
    nameEn: "Tea",
    nameAm: "ጥቁር ሻይ",
    nameOr: "Shaayee Idilee",
    category: "hot_drink",
    price: 35,
    isAvailable: true,
    placeholderSlot: "menu-black-tea.jpg",
    imageUrl: "https://images.unsplash.com/photo-1544787219-7f47ccb76574?auto=format&fit=crop&w=1200&q=85",
    description: "Steaming hot fragrant black tea served with fresh mint sprig and lemon slice on the side.",
  },
  {
    id: "hot-8",
    nameEn: "Ginger tea",
    nameAm: "የዝንጅብል ሻይ",
    nameOr: "Shaayee Jinjibilaa",
    category: "hot_drink",
    price: 70,
    isAvailable: true,
    placeholderSlot: "menu-ginger-tea.png",
    imageUrl: "/images/menu-ginger-tea.png",
    description: "Soothing natural tea steeped with fresh crushed spicy ginger roots and lemon.",
  }
];
